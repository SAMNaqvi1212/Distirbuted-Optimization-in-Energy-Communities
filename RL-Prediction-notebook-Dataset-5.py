...
# # Reinforcement Learning Model Dataset-5

# This notebook takes the saved Reinforcement Learning model that is trained and then implemented on the dataset-5. The agent is loaded in this notebook and then predicts while using the dataset-5. 
...
def get_data(start = '2017-01-01 00:00:00', end = '2017-03-01 23:55:00'):
    ...
    slp = pd.read_csv('df_p.csv', index_col=0, parse_dates=True)['5'] / 1000
...
#Test the model on the dataset using the model and record them in separate lists. 
env = CostEnv()
obs = env.reset()
done = False
while not done:
...
#Plot the graphs for the dataset-5 values. 
...
plt.title("Grid Optimization for Dataset-5")
...
### Saving the dataframe to plot the values for the paper. 
...
df_5 = pd.DataFrame({
    'grid_values_5': env.grid_values,
    'pv_values_5': env.pv_values,
    'battery_state_values_5': env.battery_state_values
}, index=date_range)
...
df_5.to_csv(directory + '5_dataframe.csv', index=True)
df_5.to_csv(r'Desktop/tootoo/Datasets/Reinforcement-Learning/5_dataframe.csv',index=True)
